...
class TPACScraper(BaseScraper):
    def __init__(self, url: str = None, scrape_job_id: str = None,
        ...
        self.extractor = TPACExtractor(config=self.config)
        self.processor = TPACProcessor(config=self.config)
        ...
        if self._event_tracker:
            ...
            if self.scraper_definition and hasattr(self.scraper_definition, 'display_name'):
                ...
                venue_name = self.config.get('venue_name', 'TPAC')
            self._event_tracker.venue = venue_name

    @property
    def name(self) -> str:
        return "tpac_scraper_v1"

    async def extract_data(self) -> Dict[str, Any]:
        try:
            performance_data, seat_data = await self.extractor.extract(self.url)

            if not seat_data:
                error_msg = "Failed to extract required TPAC seat data"
                if self._event_tracker:
            ...
            if self._event_tracker:
                screens_count = len(seat_data.get('screens', []))
                seat_lists_count = len(seat_data.get('seat_lists', {}))
                performance_details = performance_data.get('performance_details', {})
                venue_title = performance_details.get('facility_desc', 'Unknown')
                
                self._event_tracker.track_status_update(
                    f"Extracted data: {screens_count} screens, {seat_lists_count} seat lists",
                    metadata={
                        'screens_count': screens_count,
                        'seat_lists_count': seat_lists_count,
    ...
    async def process_data(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            seat_data = raw_data["seat_data"]

            scraped_data = self.processor.process(performance_data, seat_data, self.url, self.scrape_job_id, self.enriched_data)
    ...
    async def store_in_database(self, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            logger.info(f"💾 Storing TPAC scraped data in database:")
            ...
            scraper_name = "tpac"  # Must match processor.py source_website
            prefix = "tp"

            # Only get prefix from scraper definition, NOT the name
            # The name "tpac_scraper_v1" would break sync algorithm
            ...
            logger.info(f"✅ TPAC database storage completed successfully")
